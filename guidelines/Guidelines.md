...
| `SidePanel` | AtudaQuestionnaireSheet | פאנל צדדי בדסקטופ, בוטום-שיט במובייל |
...
- **`Dialog` עובר ב-portal** לשורש האפליקציה (`[data-app-root]`).
  ...
  לממוקם ביחס אליו, והחלון היה נפתח בתוך הכרטיס. אותו כלל חל על
  `SidePanel`.
- **`Dialog` מול `SidePanel`**: `Dialog` לאישור, טופס קצר או סינון -
  אינטראקציה שנגמרת במשפט אחד. `SidePanel` לבחירה מתוך מאגר גדול
  שצריכה מקום: חיפוש, רשימה מקובצת, מידע הסבר ובחירה מרובה - כשחשוב
  שהעמוד שמאחור יישאר בהקשר (דסקטופ) או שהמסך יתפנה למשימה אחת
  (מובייל).
...
**פאנל בחירה** (שאלון עתודה): אותו תוכן בשתי מעטפות לפי `useIsMobile`
(גבול 768) -

- **דסקטופ**: פאנל נצמד לימין המסך, `460px` (`max-w-[92vw]`), גובה
  מלא, נכנס ב-`translateX(100%) → 0` ב-300ms, צל `ELEVATION.overlay`.
- **מובייל**: בוטום-שיט בגובה `88vh` עם פינות עליונות `rounded-t-2xl`,
  ידית אחיזה אפורה (`w-10 h-1`) ומעבר `translateY(100%) → 0`.
- **משותף**: רקע `bg-black/20` שסוגר בלחיצה, `Esc` סוגר, כותרת נעוצה
  עם שם + תת-כותרת ו-`X` בשמאל, גוף נגלל (`overflow-y-auto flex-1
  min-h-0`), ופוטר נעוץ עם "ביטול" ופעולת השמירה.
- **ניווט פנימי**: כשהפאנל מציג שני מסכים (רשימה ← פירוט), הכותרת
  מקבלת `ChevronRight` לחזרה במקום לסגור הכל; הסגירה מוותרת על
  הבחירה, השמירה מחילה אותה על השורה שממנה נפתח הפאנל.
- **מניעת כפילות**: פריט שכבר נבחר במקום אחר מוצג מושבת עם התווית
  "כבר נבחרה" - ולא נעלם מהרשימה.

**מכסות ומגבלות בבחירה מרובה**: מונעים שגיאה במקום לדווח עליה - בהגעה
למכסה שאר האפשרויות מעומעמות (`opacity-40`) ומושבתות, ומתחתן שורת
הסבר אפורה איך בכל זאת להחליף. לחיצה חוזרת על פריט שנבחר מסירה אותו
והמספור מתעדכן אוטומטית. כפתור המשך לשלב הבא מושבת עד שהתנאים
מתקיימים, עם שורת הסבר שנעלמת ברגע שאפשר להמשיך - ולא הודעת שגיאה
אדומה אחרי לחיצה. אדום נשמר לשגיאה אמיתית שהמשתמש כבר נתקל בה.
...
מענה / פתיחה מחדש / צירוף מסמך. מוסתרת כרגע מאחורי `ADMIN_VIEW_ENABLED`,
כמו מתג גרסאות השאלון מאחורי `VARIANT_SWITCH_ENABLED` - מתגי השוואה
נשארים בקוד מאחורי דגל אחד ולא נמחקים.
